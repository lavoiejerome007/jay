...
def get_stop_loss_atr(ticker_symbol):
    try:
        ...
        # Stop loss basé sur 2 fois l'ATR
...
def show_system2():
    ...
    with tab_view:
        if my_trans.empty:
            ...
            # ----- NOUVEAU: CROISSANCE ENTRE DEUX DATES -----
            st.divider()
    ...
    with tab_strategy:
            ...
            if st.session_state.get("last_strategy_data"):
                ...
                if st.button("✅ Vendre mes actions actuelles et Acheter cette stratégie"):
                    with st.spinner("Mise à jour du portefeuille en cours..."):
                        # Vente des actions actuelles pour garder l'historique
                        holdings = {}
                        ...
                        # Achat de la nouvelle stratégie
        ...
        # ----- NOUVEAU: AJOUT DE FONDS SUPPLEMENTAIRES -----
        st.divider()
